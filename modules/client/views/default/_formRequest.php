...
<div class="request-form" novalidate class="form mt-2">
    ...
    <div class="container container-xs px-3">
        ...
        <?= $form->field($model,
            'id_city')->label(false)->hiddenInput(['value' => Yii::$app->user->identity->id_city]); ?>

        <?= $form->field($model,
            'status_request')->label(false)->hiddenInput(['value' => \app\models\Request::STATUS_METERING_BEFORE]) ?>
        ...
        <a href="/client/default/request-new-metering-date">
            <button type="button" class="btn btn-primary btn-block">Дата: <?= Yii::$app->request->POST('date'); ?>
                Время: <?= str_replace(';', ':00 ', Yii::$app->request->POST('my_range')) . ':00'; ?></button>
        </a>

        <span class="input-nao">
                <?= $form->field($model, 'address_client_street',
                    ['template' => "{input}\n{label}\n $svg \n{error}{hint}"])->textInput(['class' => 'input-nao__field valid'])->label('Укажите улицу',
                    ['class' => 'input-nao__label', 'for' => "address"]) ?>
            </span>
        <span class="input-nao">
                <?= $form->field($model, 'address_client_house',
                    ['template' => "{input}\n{label}\n $svg \n{error}{hint}"])->textInput(['class' => 'input-nao__field valid'])->label('Укажите номер дома',
                    ['class' => 'input-nao__label', 'for' => "address"]) ?>
            </span>

        <span class="input-nao">
                <?= $form->field($model, 'address_client_room',
                    ['template' => "{input}\n{label}\n $svg \n{error}{hint}"])->textInput(['class' => 'input-nao__field valid'])->label('Укажите номер квартиры',
                    ['class' => 'input-nao__label', 'for' => "address"]) ?>
            </span>
